                ...
                <div class="col-10">
                    <select class="form-control" id="supplier_id" name="supplier_id" required>
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->supplier_id }}" {{ old('supplier_id', $stok->supplier_id) == $supplier->supplier_id ? 'selected' : '' }}>
                ...
                <div class="col-10">
                    <select class="form-control" id="user_id" name="user_id" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->user_id }}" {{ old('user_id', $stok->user_id) == $user->user_id ? 'selected' : '' }}>
                                {{ $user->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-10">
                    <select class="form-control" id="barang_id" name="barang_id" required>
                        ...
                        @foreach($barangs as $barang)
                            <option value="{{ $barang->barang_id }}" {{ old('barang_id', $stok->barang_id) == $barang->barang_id ? 'selected' : '' }}>
                                {{ $barang->barang_nama }}
                            </option>
                        @endforeach
                    </select>
